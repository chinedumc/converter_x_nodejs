...
const fileFilter = (req, file, cb) => {
  ...
  if (config.ALLOWED_EXTENSIONS.includes(ext)) {
    ...
    cb(new Error(`Invalid file type. Only ${config.ALLOWED_EXTENSIONS.join(', ')} files are allowed`), false);
  }
};
...
router.post('/validate', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    logger.info(`File upload attempt: filename=${req.file.originalname}, content_type=${req.file.mimetype}`);
    ...
    logger.error(`File upload failed: filename=${req.file?.originalname}, error=${error.message}`);
...
router.post('/convert', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    logger.error(`Conversion failed for file: ${req.file?.originalname} - ${error.message}`);
    res.status(500).json({
...
router.get('/download/:fileId', async (req, res) => {
  ...
  try {
    ...
    logger.error(`File download failed: filename=${fileId}, error=${error.message}`);
