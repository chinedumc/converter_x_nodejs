...
class ExcelToXMLConverter {
  ...
  _createHeader(root, headerFields) {
    ...
    for (const [key, value] of Object.entries(headerFields)) {
      try {
        ...
        header.ele(tagName).txt(value !== null && value !== undefined ? String(value) : '');
      } catch (error) {
        throw new Error(`Error processing header field ${key}: ${error.message}`);
      }
    }
  }
    ...
    return jsonData.map(row => {
      ...
      for (const [column, value] of Object.entries(row)) {
        ...
        if (value === null || value === undefined || value === '') {
          ...
          record[column] = value.toISOString();
   ...
   */
  convert(inputFile, outputFile, headerFields = null, sheetName = null, encryptOutput = false, userId = 'system') {
